...
def load_taxonomy_file(taxonomy_path: str | Path) -> Dict[str, Any]:
    ...
    Raises:
        TaxonomyError: If the taxonomy file doesn't exist or cannot be accessed
...
def _is_nested_taxonomy(taxonomy_data: Dict[str, Any]) -> bool:
    ...
    if isinstance(first_subject, dict):
        ...
        return isinstance(first_topic, dict) and (
            "level" in first_topic or "description" in first_topic
        )
...
def get_random_topic_with_level(
    taxonomy_data: Dict[str, Any], difficulty_level: str = None
    ...
    """
    # random imported at top of module

...
class CostTracker:
    ...
    def log(self, model_config: Dict[str, str], input_tokens: int, output_tokens: int) -> None:
        ...
        input_cost = (input_tokens / _TOKEN_DIVIDER) * input_rate
        output_cost = (output_tokens / _TOKEN_DIVIDER) * output_rate
    ...
    def get_total_cost(self) -> float:
        """Return the total cost rounded to 6 decimal places."""
        with self._lock:
            total = self._total_cost
        return round(total, 6)

    def get_breakdown(self) -> Dict[str, Dict[str, float]]:
        """Return breakdown of costs per model."""
        with self._lock:
            stats_copy = dict(self._model_stats)
        return {
            f"{provider}:{model}": {
            ...
            for (provider, model), stats in stats_copy.items()
        }

    def as_dict(self, run_id: Optional[str] = None) -> Dict[str, Any]:
        """Return dictionary representation of the cost tracker."""
        return {
